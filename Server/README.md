...
Production-grade FastAPI backend for Architecture Intelligence platform with **LangGraph reasoning** and **FastMCP server**.
...
- **Rule-Based Issue Detection**: Deterministic detection with 5 rule types
- **LangGraph Reasoning Pipeline**: Non-linear workflow generation with conditional branching
- **Workflow Generation**: 3 alternatives (Minimal, Performance, Cost) via LangGraph
- **FastMCP Server**: MCP-compliant interface for AI agent integration
- **REST APIs**: Clean, production-ready endpoints
...
├── reasoning/             # LangGraph reasoning
│   ├── rules.py           # Deterministic rules
│   ├── graph_analysis.py  # Graph algorithms
│   └── langgraph_pipeline.py  # Non-linear orchestration
├── mcp/                   # FastMCP server
...
Returns 3 workflow alternatives generated by **LangGraph pipeline**:
...
Non-linear workflow generation with conditional branching:
...
- Issue categorization (performance, reliability, scalability)
- Strategy selection based on issue types
- Parallel workflow generation
- Conditional routing based on issue severity
...
- `get_current_architecture()` - Get graph with metrics
- `get_detected_issues()` - Get all issues with categorization
- `generate_workflows()` - Generate 3 workflows via LangGraph
- `compare_workflows()` - Compare with recommendation
- `explain_decision(workflow_id)` - Explain reasoning
- `get_graph_analysis()` - Advanced graph analysis (centrality, bottlenecks, cycles)
...
2. **Deterministic core**: Rules work without LLMs
3. **LangGraph orchestration**: Non-linear reasoning flow
4. **MCP-ready**: JSON serializable, stateless services
5. **Pydantic everywhere**: Strict validation
6. **Async by default**: FastAPI async patterns
7. **Production-grade**: Proper logging, error handling, validation
...
- **Single Point of Failure**: In-degree > 5
...
- [x] LangGraph reasoning pipeline
